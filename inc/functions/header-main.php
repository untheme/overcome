 ...
 * Header Main 
*/
if(!function_exists('unbreak_header_main')){
    function unbreak_header_main($class = ''){
 ...
 * Header Class 
**/
if(!function_exists('unbreak_header_class')){
    function unbreak_header_class($class = ''){
        $classes = [];
        $classes[] = 'ef5-header';
...
if(!function_exists('unbreak_header_inner_class')){
    function unbreak_header_inner_class($class = ''){
        $header_fullwidth = unbreak_get_opts('header_fullwidth', '0');
...
if(!function_exists('unbreak_header_ontop')){
    function unbreak_header_ontop(){
        $header_ontop = unbreak_get_opts('header_ontop');
        return  $header_ontop;
    }
}

if(!function_exists('unbreak_header_sticky')){
    function unbreak_header_sticky(){
        $header_sticky = unbreak_get_opts('header_sticky');
        return  $header_sticky;
    }
}

if(!function_exists('unbreak_header_menu')){
    function unbreak_header_menu($args = []){
        $args = wp_parse_args($args,[
            'class' => ''
        ]);
        $header_menu = unbreak_get_opts('header_menu','ef5-primary');
...
if(!function_exists('unbreak_header_side_menu')){
    function unbreak_header_side_menu($args = []){
        $args = wp_parse_args($args,[
            'class' => ''
        ]);
        $header_menu = unbreak_get_opts('header_menu','ef5-primary');
...
if(!function_exists('unbreak_header_menu_fallback')){
    function unbreak_header_menu_fallback(){
        printf(
            '<ul id="ef5-header-menu" class="%1$s"><li class="menu-item required"><a href="%2$s">%3$s</a></li></ul>',
            esc_attr(unbreak_header_menu_class('primary-menu-not-set')),
            esc_url( admin_url( 'nav-menus.php?action=locations' ) ),
            esc_html__( 'Primary menu is not set, please location to "Primary Menu".', 'unbreak' )
        );
    }
}
 ...
 *
*/
if(!function_exists('unbreak_header_menu_class')){
    function unbreak_header_menu_class($class=''){
        $classes = ['ef5-menu'];
        $header_layout = unbreak_get_opts('header_layout', '1');
        $header_ontop  = unbreak_get_opts('header_ontop','0');
        $header_sticky = unbreak_get_opts('header_sticky','0');
        $header_helper_menu = unbreak_get_opts('header_helper_menu','');
