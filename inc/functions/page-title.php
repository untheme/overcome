 ...
 * Page title Layout
*/
function unbreak_page_title(){
    $ptitle_layout = unbreak_get_opts('ptitle_layout', '2');
...
*/
function unbreak_ptitle_inner_class($class=''){
	$classes = ['ef5-pagetitle-inner'];
	$full = unbreak_get_opts('ptitle_full_width', '0');
	if($full === '1')
 ...
 */
function unbreak_breadcrumb($args = [])
{
    if ( ! class_exists( 'UnBreak_Breadcrumb' ) )
    {
        return;
    }
    ...
    $breadcrumb = new UnBreak_Breadcrumb();
    ...
    $separator = apply_filters('unbreak_breadcrumb_separator', $args['separator']);
    ob_start();
    $count = count($entries);
    $d = 0;
...
*/
function unbreak_ptitle_parallax_image(){
    $parallax_url = unbreak_get_opts('ptitle_parallax',['url'=> '']);
