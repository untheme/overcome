...
function overcome_get_the_posts_pagination( $args = array() ) {
    $navigation = '';
    ...
    if ( $GLOBALS['wp_query']->max_num_pages > 1 ) {
        ...
        $args = wp_parse_args(
            ...
            array(
                'mid_size'           => 1,
                'prev_text'          => _x( 'Previous', 'previous set of posts' ,'overcome' ),
                'next_text'          => _x( 'Next', 'next set of posts' ,'overcome' ),
                'screen_reader_text' => esc_html__( 'Posts navigation' ,'overcome' ),
                'aria_label'         => esc_html__( 'Posts' ,'overcome' ),
                'class'              => ''
            )
        );
...
function overcome_the_posts_pagination( $args = array() ) {
    echo overcome_get_the_posts_pagination($args);
}
...
if(!function_exists('overcome_post_navigation')){
    function overcome_post_navigation($args = []){
        $args = wp_parse_args($args, [
            'layout' => '1',
            'class'  => 'row justify-content-center gutters-lg-100'
        ]);
        $navigation = get_the_post_navigation();

        $post_type_name = str_replace(['ef5_', 'ef5-', 'tribe-'], '', get_post_type());
        $post_type_name = str_replace(['ies'], 'y', $post_type_name);

        $previous = get_previous_post_link(
            '<div class="col-auto"><div class="nav-previous">%link</div></div>',
            '<span class="meta-nav" data-title="%title"><span class="nav-icon"></span>'.esc_html__('Prev','overcome').' '.$post_type_name.'</span>'
        );
        $next = get_next_post_link(
            '<div class="col-auto"><div class="nav-next text-end">%link</div></div>',
            '<span class="meta-nav" data-title="%title">'.esc_html__('Next','overcome').' '.$post_type_name.'<span class="nav-icon"></span></span>'
        );
