 ...
 * add_action('switch_theme', 'unbreak_update_avatar');
 *
*/
function unbreak_update_avatar(){
 ...
 * add_filter( 'avatar_defaults', 'unbreak_default_avatar' );
 */
function unbreak_default_avatar( $avatar_defaults ) {
	$ef5_avatar = get_template_directory_uri() . '/assets/images/avatar.png';
	$avatar_defaults[$ef5_avatar] = esc_html__('UnBreak Avatar','unbreak');
 ...
 *
 */
if(!function_exists('unbreak_get_avatar_size')){
	function unbreak_get_avatar_size() {
		return unbreak_configs('cmt_avatar_size');
	}
}
 ...
 * add_filter( 'get_avatar' , 'unbreak_custom_avatar' , 1 , 6 );
 * 
*/
if(!function_exists('unbreak_custom_avatar')){
	function unbreak_custom_avatar($avatar, $id_or_email, $size, $default, $alt, $args) {
