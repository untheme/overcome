...
if ( ! function_exists( 'unbreak_posted_by' ) ) :
    /**
     * Prints HTML with meta information for the current post author.
     */
    function unbreak_posted_by($args=[]){
        global $post;
        $author_id   = $post->post_author;
        $args = wp_parse_args($args, [
            'class'              => '',
            'hint'               => esc_html__( 'Posted by', 'unbreak' ),
            'icon'               => '',
            'author_avatar'      => true,
            'before_author_name' => esc_html__( 'By', 'unbreak' ).'&nbsp;&nbsp;',
            'after_author_name'  => '',
            'show_author'        => '1',
            'echo'               => true   
        ]);
...
if ( ! function_exists( 'unbreak_posted_on' ) ) :
    /**
     * Prints HTML with meta information for the current post-date/time.
     */
    function unbreak_posted_on($args=[])
    {
        $args = wp_parse_args($args,[
            'class'       => '',
            'hint'        => esc_html__( 'Published on', 'unbreak' ),
            'hint_update' => esc_html__( 'Updated on', 'unbreak' ),
            'icon'        => '',
            'icon_update' => '',
            'before_date' => '',
            'after_date'  => '',
...
if ( ! function_exists( 'unbreak_posted_in' ) ) :
    /**
     * Prints HTML with meta information for the current post-date/time.
     */
    function unbreak_posted_in($args = []){
        $args = wp_parse_args($args, [
            'class'           => '',
            'hint'            => esc_html__( 'Posted in', 'unbreak' ),
            'icon'            => '',
            'before'          => '',
            'after'           => '',
            'before_category' => '',
        ...
        $taxo = unbreak_get_post_taxonomies();
...
if ( ! function_exists( 'unbreak_tagged_in' ) ) :
    /**
     * Prints HTML with meta information for the current post-date/time.
     */
    function unbreak_tagged_in($args = []){
        $args = wp_parse_args($args, [
            'class'      => '',
            'hint'       => esc_html__( 'Tagged in', 'unbreak' ),
            'icon'       => 'fa fa-tag accent-color icon-rotateY-180',
            'before'     => '',
            'after'      => '',
            'before_tag' => '<span class="ef5-heading text-uppercase font-style-500">'.esc_html__('Tags:','unbreak').'</span>',
            'after_tag'  => '',
            'sep'        => ', ',
            'show_tag'   => is_single() ? unbreak_get_theme_opt( 'post_tags_on', '1' ) : unbreak_get_theme_opt( 'archive_tags_on', '1' ),
            'echo'       => true
        ]);
        if('1' !== $args['show_tag']) return;
        $classes = ['ef5-tagged-in hint--top', $args['class']];
        $tags_list = get_the_term_list( get_the_ID(), unbreak_get_post_taxonomies('tag'), '', $args['sep'], '' );
        ob_start();
            if ( $tags_list)
            {
                printf(
 ...
 */
if ( ! function_exists( 'unbreak_comments_popup_link' ) ) {
    function unbreak_comments_popup_link($args = [])
    {
        $args = wp_parse_args($args, [
            'class'     => '',
            'before'    => '',
        ...
        if ( ! post_password_required() && ( comments_open() || get_comments_number() ) )
        {
            ...
            if(!$args['show_text']){
                comments_popup_link(
                    sprintf('<span class="hint--top" data-hint="%s">%s %s</span>',esc_html__('Be the first to comment','unbreak'), $args['icon'], $number),
                    sprintf('<span class="hint--top" data-hint="%s">%s %s</span>',esc_html__('Post a comment','unbreak'), $args['icon'], $number),
                    sprintf('<span class="hint--top" data-hint="%s">%s %s</span>',esc_html__('Post a comment','unbreak'), $args['icon'], $number)
                );
            } else {
                comments_popup_link(
                    sprintf('<span class="hint--top" data-hint="%s">%s %s %s</span>',esc_html__('Be the first to comment','unbreak'), $args['icon'], $number, esc_html__('Comments','unbreak')),
                    sprintf('<span class="hint--top" data-hint="%s">%s %s %s</span>',esc_html__('Post a comment','unbreak'), $args['icon'], $number, esc_html__('Comment','unbreak')),
                    sprintf('<span class="hint--top" data-hint="%s">%s %s %s</span>',esc_html__('Post a comment','unbreak'), $args['icon'], $number, esc_html__('Comments','unbreak'))
                );
            }
            printf ('%s' , $args['after']);
            echo '</div>';
...
*/
function unbreak_post_count_view($args=[]){
    $args = wp_parse_args($args, [
        'show_view' => '1',
        'class'     => '',
        'before'    => '',
        ...
        echo '<div class="'.esc_attr($classes).'">';
            printf ('%s' , $args['before']);
                printf(
                    ...
                    $args['show_text'] ? esc_html__('View','unbreak') : '',
                    $args['show_text'] ? esc_html__('Views','unbreak') : ''
                );
            printf ('%s' , $args['after']);
        echo '</div>';
 ...
 * Show count of viewed 
*/
function unbreak_post_count_like($args=[]){
    $args = wp_parse_args($args, [
        'show_like' => '1',
        'class'     => '',
        'before'    => '',
        ...
        echo '<div class="'.esc_attr($classes).'">';
            printf ('%s' , $args['before']);
                printf(
                    ...
                    $args['show_text'] ? esc_html__('Like','unbreak') : '',
                    $args['show_text'] ? esc_html__('Likes','unbreak') : ''
                );
            printf ('%s' , $args['after']);
        echo '</div>';
 ...
 */
function unbreak_edit_link($args = [])
{
    $args = wp_parse_args($args, [
        'class'     => '',
        'icon'      => 'far fa-edit',
        'title'     => esc_html('Edit', 'unbreak'),
        'hint'      => esc_html('Edit', 'unbreak'),
        'before'    => '',
        'after'     => '',
        'show_edit' => false,
        'echo'      => true
...
function unbreak_link_pages()
{
    wp_link_pages( array(
        'before'      => sprintf( '<div class="page-links clearfix"><span class="ef5-heading">%s</span>', esc_html__( 'Pages:', 'unbreak' ) ),
        'after'       => '</div>',
 ...
 * Post Share
*/
if(!function_exists('unbreak_post_share')){
    function unbreak_post_share($args = array()){
        wp_enqueue_script('sharethis');
        global $post;
        $defaults = array(
            'show_share'  => is_single() ? unbreak_get_theme_opt( 'post_share_on', '0' ) : unbreak_get_theme_opt( 'archive_share_on', '0' ),
            'class'       => '',
            'show_title'  => true,
            'title'       => esc_html__('Share this article','unbreak'),
            'social_args' => [],
            'echo'        => true
        );
        ...
        if(is_singular()){
            $show_fb    = unbreak_get_theme_opt( 'post_share_fb', '1' );
            $show_tw    = unbreak_get_theme_opt( 'post_share_tw', '1' );
            $show_gplus = unbreak_get_theme_opt( 'post_share_gplus', '1' );
            $show_pin   = unbreak_get_theme_opt( 'post_share_pin', '1' );
            $show_all   = unbreak_get_theme_opt( 'post_share_all', '1' );
        } else {
            $show_fb    = unbreak_get_theme_opt( 'archive_share_fb', '1' );
            $show_tw    = unbreak_get_theme_opt( 'archive_share_tw', '1' );
            $show_gplus = unbreak_get_theme_opt( 'archive_share_gplus', '1' );
            $show_pin   = unbreak_get_theme_opt( 'archive_share_pin', '1' );
            $show_all   = unbreak_get_theme_opt( 'archive_share_all', '1' );
        }
        ob_start();
...
*/
if ( ! function_exists( 'unbreak_post_read_more' ) ) {
    /**
     * Prints post read more link
     */
    function unbreak_post_read_more($args = [])
    {
        $args = wp_parse_args($args,[
            'class'          => '',
            'icon_left'      => '',
            ...
            'show_readmore'  => unbreak_get_theme_opt( 'archive_readmore', '1' ),
            'before'         => '',
            'after'          => '',
            'echo'           => true
        ]);
        ...
        ob_start();
            printf(
                ...
                unbreak_optimize_css_class(implode(' ', $classes)),
 ...
 * Prints post read more link
*/
if ( ! function_exists( 'unbreak_post_read_more_circle' ) ) {
    function unbreak_post_read_more_circle($args = [])
    {
        $args = wp_parse_args($args,[
            'class'          => '',
            'bgcolor'        => 'bg-accent',
