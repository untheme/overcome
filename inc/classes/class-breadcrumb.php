...
class UnBreak_Breadcrumb
{
    ...
    function __construct( $args = array() )
    {
        $args = wp_parse_args( $args, array(
            'home_label'            => esc_html__( 'Home', 'unbreak' ),
            '404_label'             => esc_html__( '404', 'unbreak' ),
            'search_results_label'  => esc_html__( 'Search Results', 'unbreak' ),
            'entry_max_length'      => 7,
    ...
    function add_tag_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Tag:', 'unbreak' ).' '.single_tag_title( '', false ) ).' '.get_tag_link( $queried_object->term_id
        );
    }
    ...
    function add_author_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Author:', 'unbreak' ).' '.$userdata->display_name
        );
    }
    ...
    function add_tax_entry()
    {
         ...
         */
        $entries = apply_filters( 'unbreak_breadcrumb_taxonomy', $entries, $this_term );
    ...
    function add_paged_entry()
    {
        ...
        {
            $this->add_entry(
                apply_filters( 'unbreak_breadcrumb_paged', sprintf( 'Page %s', $page ) ),
                ''
            );
        }
    }
    ...
    function add_single_ancestor_entry( $post )
    {
         ...
         */
        $entries = apply_filters( 'unbreak_breadcrumb_single', $entries, $post );
