				...
				<div class="col-lg-8 ef5-tribe-event-content">					
					...
					<nav class="ef5-single-tribe-events-nav-pagination" aria-label="<?php printf( esc_html__( '%s Navigation', 'overcome' ), $events_label_singular ); ?>">
						<ul class="tribe-events-sub-nav">
							<li class="tribe-events-nav-previous"><?php tribe_the_prev_event_link( '<span></span>'.esc_html__('Prev Event','overcome') ) ?></li>
							<li class="tribe-events-nav-next"><?php tribe_the_next_event_link( esc_html__('Next Event','overcome').'<span></span>' ) ?></li>
						</ul>
					</nav>
					<div class="ef5-single-event-cmt"><?php
						if ( get_post_type() == Tribe__Events__Main::POSTTYPE && tribe_get_option( 'showComments', false ) ) { 
							comments_template('/comments-tribe-event.php');
						}
					?></div>
				</div>
