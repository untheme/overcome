...
<div class="ef5-social-wraps">
    ...
    <?php
        if(!empty($el_title)) echo overcome_html($title);
        switch ($source) {
            case 'custom':
                foreach($values as $value){
                    ...
                    $link_open = '<a href="javascript:void(0)" data-hint="'.esc_html__('Follow Us','overcome').'">';
                    $link_close = '</a>';           
                    if (isset($value['icon_link'])){  
                        ...
                        if ( strlen( $link['url'] ) > 0 ) {
                            $a_href    = $link['url'];
                            $a_title   = isset($link['title']) && !empty($link['title']) ? $link['title'] : esc_html__('Follow Us','overcome');
